...
export const brandData = {
  ...
  source: {
    ...
    note:
      "Summarized from the readable rules in the supplied Clever brand guideline deck. The source deck's example photos and illustrations are not redistributed here."
  },
  positioning: {
  ...
  colors: {
    primary: [
      ...
      { name: "White", hex: "#FFFFFF", rgb: "255, 255, 255", usage: "Primary background and reversed logo fill." },
      { name: "Dark navy", hex: "#0A1E46", rgb: "10, 30, 70", usage: "Dark background and high-contrast brand field." }
    ] satisfies ColorToken[],
    secondary: [
      { name: "Black", hex: "#1C1C1C", rgb: "28, 28, 28", usage: "Text color only." },
      { name: "Light blue", hex: "#DAEBFF", rgb: "218, 235, 255", usage: "Light background fill." },
      { name: "Yellow", hex: "#FFE478", rgb: "255, 228, 120", usage: "Shape fill and tag color." },
      { name: "Orange", hex: "#F78239", rgb: "247, 130, 57", usage: "Shape fill and tag color." },
      { name: "Green", hex: "#4ECC97", rgb: "78, 204, 151", usage: "Shape fill and tag color." }
    ] satisfies ColorToken[],
    productUi: [
      { name: "UI black", hex: "#1C1C1C", usage: "Product text and foreground." },
      { name: "UI gray", hex: "#D3D6DE", usage: "Foreground support." },
      { name: "UI light blue", hex: "#DAEBFF", usage: "Foreground support." },
      { name: "UI periwinkle", hex: "#A8C9FF", usage: "Foreground support." },
      { name: "UI navy", hex: "#0A1E46", usage: "Foreground support." },
      { name: "Portal header blue", hex: "#1464FF", usage: "Always use for the portal header." },
      { name: "UI green", hex: "#4ECC97", usage: "Foreground or alert color." },
      { name: "UI orange", hex: "#F78239", usage: "Foreground support." },
      { name: "UI yellow", hex: "#FFE478", usage: "Foreground support." },
      { name: "Alert mint", hex: "#B8EBD5", usage: "Alert support color." },
      { name: "Alert red", hex: "#CC2F2F", usage: "Alert color." },
      { name: "Background gray", hex: "#C2C7D1", usage: "Product background." },
      { name: "Background mist", hex: "#EBECF2", usage: "Product background." },
      { name: "Background white-blue", hex: "#F4F8FF", usage: "Product background." }
    ] satisfies ColorToken[]
  },
  sections: [
    {
    ...
    },
    {
      ...
      body: [
        ...
        "For product UI, use Clever blue #1464FF for the portal header. Use color intentionally to highlight important features. Avoid using every color at once."
      ]
    },
    {
      key: "typography",
      ...
      body: [
        ...
        "Fallback font pairs, only when brand fonts are unavailable: Merriweather with Inter, PT Serif with Manrope, or Martel with Plus Jakarta Sans."
      ]
    },
    {
      key: "photography",
...
Extracted: ${brandData.source.extractedOn}
...
export function renderRobotPrompt(baseUrl = "https://<your-clever-brand-identity-domain>"): string {
...
- ${baseUrl}/llms.txt
- ${baseUrl}/brand-guidelines.md
- ${baseUrl}/brand-guidelines.json
...
export function renderLlmsTxt(baseUrl = ""): string {
...
- [Brand guidelines markdown](${path("/brand-guidelines.md")}): Agent-readable brand rules.
- [Brand guidelines JSON](${path("/brand-guidelines.json")}): Structured brand tokens and sections.
- [Robot prompt](${path("/api/instructions?format=prompt")}): Pasteable system/developer prompt for app-building agents.
- [MCP endpoint](${path("/api/mcp")}): Streamable HTTP MCP server with brand resources, prompts, and tools.
- [Human docs](${path("/docs/robot-instructions.md")}): Setup instructions for people and robots.
...
function renderColorList(colors: readonly ColorToken[]): string {
  return colors.map((color) => `- ${color.name}: ${color.hex}${color.rgb ? `, RGB ${color.rgb}` : ""}. ${color.usage}`).join("\n");
}
